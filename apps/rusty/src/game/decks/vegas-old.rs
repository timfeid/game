...
use crate::game::action::{
    Action, AsyncClosureAction, BlankAction, CardAction, CardActionWrapper, CardTargetTeam,
...
fn create_high_roller() -> Card {
    CardBuilder::new()
        ...
        .add_action(
            ActionBuilder::new(ActionTriggerType::AbilityWithinPhases(
                "Remove 1 Luck couter: Flip a coin. If heads, High Roller gets +2/+2 until end of turn. If tails, High Roller gets -1/-1 until end of turn."
                    .to_string(),
                vec![],
                None,
                false,
...
fn create_dealers_enforcer() -> Card {
    CardBuilder::new()
        .name("Dealer's Enforcer, Blackjack Bruiser")
        .creature(1, 1)
        .mana_cost(vec![ManaType::Colorless, ManaType::Blue, ManaType::Blue])
        .add_action(
            ActionBuilder::new(ActionTriggerType::AbilityWithinPhases(
                "Destroy target tapped creature. Activate only during your turn, before attackers are declared.".to_string(),
                vec![],
                Some((vec![
                    TurnPhase::Untap,
                    TurnPhase::Upkeep,
                ...
                true,
                CardRequiredTarget::CardOfType(CardType::Creature, CardTargetTeam::Any, Some(true)),
            ))
            .closure_action(|game, source, owner, target, ability_id| {
                Box::pin(async move {
                    let card = Game::card_from_frontend_target(&game, &target.ok_or_else(|| format!("Please select a target"))?).await;
...
fn create_mind_games() -> Card {
    CardBuilder::new()
        .name("Mind Games")
        .description("Counter target spell unless its controller pays {2C}.")
        .card_type(CardType::Instant)
        .mana_cost(vec![ManaType::Colorless, ManaType::Blue])
...
fn create_blackjack_dealer() -> Card {
    CardBuilder::new()
        ...
        .add_action(
            ActionBuilder::new(ActionTriggerType::AbilityWithinPhases(
                "Target player discards a random card.".to_string(),
                vec![ManaType::Blue],
                None,
                true,
...
//         .add_action(
//             ActionBuilder::new(ActionTriggerType::AbilityWithinPhases(
//                 "Adds {D} to your mana pool as long as you have at least 1 Luck Token.".to_string(),
//                 vec![],
//                 None,
//                 true,
//                 CardRequiredTarget::None,
...
//         .add_action(
//             ActionBuilder::new(ActionTriggerType::AbilityWithinPhases(
//                 "Adds {B} to your mana pool as long as you have at least 1 Luck Token.".to_string(),
//                 vec![],
//                 None,
//                 true,
//                 CardRequiredTarget::None,
